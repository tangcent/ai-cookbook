...
The script reads from `install/mcp-servers.json`, converts to OpenCode format (`mcp` key, `command` as array, `environment` instead of `env`, `type: "local"`), auto-fills variables from `install/.mcp-vars.json`, and merges into the existing config.
