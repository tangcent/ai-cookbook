# Install for Trae

Instructions for installing ai-cookbook items globally into Trae.

## Config Locations

| Type | Path |
|------|------|
| MCP | `~/Library/Application Support/Trae/User/mcp.json` (key: `mcpServers`) |
| Skills | `~/.trae/skills/` |

---

## Install CLIs

### playwright-cli

```bash
npm install -g @playwright/cli@latest
```

---

## Install MCPs

Merge all MCP servers into `~/Library/Application Support/Trae/User/mcp.json`:

```bash
# From the ai-cookbook root:
python3 install/trae/merge-mcp.py
```

Or merge specific servers:

```bash
python3 install/trae/merge-mcp.py github grafana
```

The script reads from `install/mcp-servers.json`, auto-fills variables from `install/.mcp-vars.json`, and merges into the existing `~/Library/Application Support/Trae/User/mcp.json` (preserving other keys). Re-running the script is safe — existing settings are never overwritten.

> For `github` and `grafana`, install the binaries first: `brew install github-mcp-server mcp-grafana`

---

## Install Skills

Install skill folders into `~/.trae/skills/` (with script path resolution):

```bash
# From the ai-cookbook root:
bash install/install-skills.sh trae
```

Or install individually:

```bash
bash install/install-skills.sh trae audit-sql
bash install/install-skills.sh trae git-workflow
```
